...
// הסרת כל ה-hooks הישנים שיוצרים בלגן
remove_action('woocommerce_order_status_changed', 'AdPro_process_order_on_status_change', 10);
remove_action('woocommerce_payment_complete', 'AdPro_process_order_on_payment_complete', 10);

/**
 * פונקציה מרכזית לעיבוד הזמנת eSIM
 * זה יהיה הקוד היחיד שמטפל בעיבוד eSIM
 */
function AdPro_process_complete_esim_order($order_id) {
    $order = wc_get_order($order_id);
    
    if (!$order) {
        error_log('AdPro eSIM: Order not found - ' . $order_id);
        return false;
    }
    ...
    if (!$has_esim) {
        error_log('AdPro eSIM: Not an eSIM order - ' . $order_id);
        return false;
    }
    
    // בדיקה אם כבר טיפלנו בהזמנה
    if ($order->get_meta('_mobimatter_completed') === 'yes') {
        error_log('AdPro eSIM: Order already processed - ' . $order_id);
        return true; // זה בסדר, ההזמנה כבר עובדה
    }
    
    // קבלת או יצירת מזהה הזמנת מובימטר
    $mobimatter_order_id = $order->get_meta('_mobimatter_order_id');
    
    if (empty($mobimatter_order_id)) {
        error_log('AdPro eSIM: Creating MobiMatter order for order #' . $order_id);
        $mobimatter_order_id = AdPro_create_mobimatter_order_internal($order, $esim_package_id);
        
        if (!$mobimatter_order_id) {
            error_log('AdPro eSIM: Failed to create MobiMatter order');
            return false;
        }
    }
    
    // השלמת הזמנה במובימטר וקבלת פרטי eSIM
    $result = AdPro_complete_mobimatter_order_internal($order, $mobimatter_order_id);
    
    if ($result) {
        // שליחת מייל ללקוח
        AdPro_send_esim_details_email($order_id);
        error_log('AdPro eSIM: Successfully processed order #' . $order_id);
        return true;
    }
    
    error_log('AdPro eSIM: Failed to complete MobiMatter order');
    return false;
}

/**
 * יצירת הזמנה במובימטר - פונקציה פנימית
 */
function AdPro_create_mobimatter_order_internal($order, $esim_package_id) {
    $api_key = get_option('AdPro_api_key');
    $merchant_id = get_option('AdPro_merchant_id');
    
    if (empty($api_key) || empty($merchant_id)) {
        $order->add_order_note('שגיאה: חסרים פרטי API של מובימטר');
        return false;
    }
    ...
    if (empty($mobimatter_order_id)) {
        $order->add_order_note('שגיאה: לא נמצא מזהה הזמנה בתשובה מובימטר');
        return false;
    }
    
    $order->update_meta_data('_mobimatter_order_id', $mobimatter_order_id);
    $order->add_order_note('נוצרה הזמנה במובימטר: ' . $mobimatter_order_id);
    $order->save();
...
/**
 * השלמת הזמנה במובימטר - פונקציה פנימית
 */
function AdPro_complete_mobimatter_order_internal($order, $mobimatter_order_id) {
    $api_key = get_option('AdPro_api_key');
    $merchant_id = get_option('AdPro_merchant_id');
    ...
    if (!isset($response_data['result'])) {
        $order->add_order_note('שגיאה: חסר שדה result בתשובה');
        return false;
    }
    ...
    // סימון שההזמנה הושלמה
    $order->update_meta_data('_mobimatter_completed', 'yes');
    $order->add_order_note('הזמנה הושלמה בהצלחה במובימטר - פרטי eSIM זמינים');
    $order->save();
...
/**
 * Hooks - נקרא מנקודות שונות כדי לוודא שההזמנה מעובדת
 */

// 1. כאשר תשלום מושלם
add_action('woocommerce_payment_complete', function($order_id) {
    error_log('AdPro eSIM: Payment completed for order #' . $order_id);
    AdPro_process_complete_esim_order($order_id);
}, 10, 1);

// 2. כאשר סטטוס הזמנה משתנה
add_action('woocommerce_order_status_changed', function($order_id, $old_status, $new_status, $order) {
    if ($new_status === 'processing' || $new_status === 'completed') {
        error_log('AdPro eSIM: Order status changed to ' . $new_status . ' for order #' . $order_id);
        AdPro_process_complete_esim_order($order_id);
    }
}, 10, 4);

// 3. כאשר הזמנה מעודכנת (גיבוי)
add_action('woocommerce_order_status_completed', function($order_id) {
    error_log('AdPro eSIM: Order completed hook for order #' . $order_id);
    AdPro_process_complete_esim_order($order_id);
}, 10, 1);

// 4. כאשר הזמנה מעודכנת ל-processing (גיבוי)
add_action('woocommerce_order_status_processing', function($order_id) {
    error_log('AdPro eSIM: Order processing hook for order #' . $order_id);
    AdPro_process_complete_esim_order($order_id);
}, 10, 1);

// 5. CRON Job גיבוי שרץ כל 15 דקות
add_action('AdPro_esim_process_pending_orders', 'AdPro_process_pending_esim_orders');

function AdPro_process_pending_esim_orders() {
    // חפש הזמנות eSIM שטרם עובדו
    $args = [
    ...
    foreach ($orders as $order) {
        ...
        if ($has_esim) {
            error_log('AdPro eSIM: Processing pending order via CRON #' . $order->get_id());
            AdPro_process_complete_esim_order($order->get_id());
        }
    }
}

// הפעלת CRON Job
if (!wp_next_scheduled('AdPro_esim_process_pending_orders')) {
    wp_schedule_event(time(), 'every_15_minutes', 'AdPro_esim_process_pending_orders');
}

// הוספת תדירות של 15 דקות
...
/**
 * פונקציה למנהל לעיבוד ידני של הזמנה (לשימוש בכלי האבחון)
 */
function AdPro_manual_process_esim_order($order_id) {
    if (!current_user_can('manage_options')) {
        return false;
    }
    
    return AdPro_process_complete_esim_order($order_id);
}
